...
from .wuxing import (
    ...
    TIAN_GAN, DI_ZHI_WUXING, DI_ZHI_WUXING,
...
# 创建 FastMCP 服务器实例
# host="0.0.0.0" 避免 mcp 自动开启 DNS rebinding protection（仅当 host 为 localhost 时才开启）
# transport_security 显式关闭 Host 校验，确保公网 IP 访问不被 421 拦截
mcp = FastMCP(
    name="每日气运值",
    host="0.0.0.0",
